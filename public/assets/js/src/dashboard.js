...
$(document).ready(function () {
    ...
    checkSidebarImage('.sidebar');
  
    $( ".sidebar-navlink" ).click(function() {
        var isCollapsed = $(this).hasClass("collapsed");
        $( ".sidebar-navlink" ).removeClass('active');
        $('.submenu-arrowr').removeClass('d-none');
        $('.submenu-arrowd').addClass('d-none');
        $('.collapse').collapse('hide');
        $(this).addClass('active');
        if (isCollapsed) {
            $(this).find(".submenu-arrowr").addClass('d-none');
            $(this).find(".submenu-arrowd").removeClass('d-none');
        } else {
            $(this).find(".submenu-arrowr").removeClass('d-none');
            $(this).find(".submenu-arrowd").addClass('d-none');
        }
    });

    $( ".sidebar-slider span" ).click(function() {
        var currVal = $('#sidebar-toggler-check').prop('checked');
        if (currVal) {
            $('#sidebar-toggler-check').prop('checked', false);
            $('.sidebar-slider span').removeClass('isLocked');
            $('#sidebar-toggle-bottom').removeClass('fa-flip-horizontal');
            $( ".sidebar-navlink" ).removeClass('active');
            $('.submenu-arrows').hide();
            $('.sidebar-wrapper .sidebar-navlink span').hide();
        }
        else {
            $('#sidebar-toggler-check').prop('checked', true);
            $('.sidebar-slider span').addClass('isLocked');
            $('#sidebar-toggle-bottom').addClass('fa-flip-horizontal');
            $('.submenu-arrows').show();
            $('.sidebar-wrapper .sidebar-navlink span').show();
        }
    });

    $( ".main-panel" ).mouseover(function() {
        var currVal = $('.sidebar-slider span').hasClass('isLocked');
        if (currVal) return;
        $('.collapse').collapse('hide');
        $('.submenu-arrowr').removeClass('d-none');
        $('.submenu-arrowd').addClass('d-none');
